...
def get_note_reseau(url):
    ...
    # Boucle à travers chaque élément 'tr' pour extraire les inreseaus pertinentes
    for tr in trs:
        ...
        if name and note:
            # Création d'une entrée de dictionnaire avec les inreseaus extraites
            entry = {"nom": name.get_text(strip=True), "reseau": note.get_text(strip=True)}
            data.append(entry)
    ...
    return df
# Fonction pour récupérer les notes de reseau à partir de plusieurs pages
