...
def get_note_international(url):
    ...
    # Boucle à travers chaque élément 'tr' pour extraire les ininternationals pertinentes
    for tr in trs:
        ...
        if name and note:
            # Création d'une entrée de dictionnaire avec les ininternationals extraites
            entry = {"nom": name.get_text(strip=True), "international": note.get_text(strip=True)}
            data.append(entry)
    ...
    return df
# Fonction pour récupérer les notes de international à partir de plusieurs pages
